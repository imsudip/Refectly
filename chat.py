        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            while True:
                ...
                def update_user_profile():
                    ...
                    profile_conversation.append(
                        {'role': 'system', 'content': open_file('system_chat_append.txt')})
                ...
                def update_knowledge_base():
                    ...
                    print('Updating KB...')
                    print('DEBUG: main_scratchpad: %s' % main_scratchpad)
                    if collection.count() == 0:
                        # yay first KB!
                        kb_convo = list()
                        ...
                        article = chatbot(kb_convo, model='gpt-3.5-turbo')
                        new_id = str(uuid4())
                        collection.add(documents=[article], ids=[new_id])
                        save_file('db_logs/log_%s_add.txt' % time(),
                                  'Added document %s:\n%s' % (new_id, article))
                    else:
                        results = collection.query(
                            query_texts=[main_scratchpad], n_results=1)
                        ...
                        save_file('db_logs/log_%s_update.txt' % time(),
                                  'Updated document %s:\n%s' % (kb_id, article))
                        ...
                        if kb_len > 1000:
                            ...
                            articles = chatbot(
                                kb_convo, model='gpt-3.5-turbo').split('ARTICLE 2:')
